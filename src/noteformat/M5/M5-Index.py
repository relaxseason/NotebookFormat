...
class BaseNoteFormat:
    ...
    def draw_gridline(
        ...
        # 描画前にドットの座標を計算
        xlist = [
            x * interval + offset + margin.width
            for x in [0, 1, 5, 6, 10]
        ]
        ylist = [
            y * interval + offset + margin.height
        ...
        for page_x, page_y in pageindex:
            self.pdf_file.setLineWidth(0.05 * mm)
            self.pdf_file.grid(
                list(map(lambda x: (x + pagewidth * page_x) * mm, xlist)),
                list(map(lambda y: (y + pageheight * page_y) * mm, ylist))
            )

        # タイトル用のマーク
        for page_x, page_y in pageindex:
            self.pdf_file.setLineWidth(0.1 * mm)
            rect_y = (pageheight - (offset + margin.height + interval))
            self.pdf_file.rect(
                ((offset + margin.width) + pagewidth * page_x) * mm,
                (rect_y + pageheight * page_y) * mm,
    ...
    def draw_nonble(
        ...
        # フォント登録
        self.set_font(10.5)
        text_width = self.pdf_file.stringWidth("00")
        text_margin_x = (interval - text_width/mm)/2
        text_margin_y = self.FIRST_DOT_INTERVAL
        # 描画前に文字列の座標を計算
        nonble = [
            (xlist, ylist)
            for xlist in [
                (x) * interval + offset + margin.width + text_margin_x
                for x in [0, 5]
            ]
            for ylist in [
                pageheight - ((y+2) * interval + offset +
                              margin.height - text_margin_y)
                for y in range(
                    math.floor(
                        (pageheight - offset * 2) / interval
                    )-1
        ...
        for page_x, page_y in pageindex:
            for n in nonble:
                index_no = "{:02}".format(nonble.index(n)+1)
                (x, y) = n
...
if __name__ == "__main__":
    ...
    format.create_format(single_color, single_color_name,
                         format.TARGET_PAGE_WIDTH, format.TARGET_PAGE_HEIGHT)
