...
class BaseNoteFormat:
    ...
    def set_property(self):
        ...
        self.pdf_file.setTitle('A6 Slim 1.25ドット方眼')
        self.pdf_file.setSubject('A6 Slim 1.25ドット方眼')
    ...
    def draw_split_line(self, color,
        ...
        # 右縦線
        right = (self.A4_LANDSCAPE_WIDTH - margin.width)
        self.pdf_file.line(
            right * mm,
            0 * mm,
            right * mm,
        ...
        # 上横線
        top = (self.A4_LANDSCAPE_HEIGHT - margin.height)
        self.pdf_file.line(
            0 * mm,
            top * mm,
            self.A4_LANDSCAPE_WIDTH * mm,
    ...
    def draw_dotgrid(self,
        ...
        pageindex = [
            (page_x, page_y)
            for page_x in range(
                math.floor(self.A4_LANDSCAPE_WIDTH / pagewidth))
            for page_y in range(
                math.floor(self.A4_LANDSCAPE_HEIGHT / pageheight))
        ]
    ...
    def create_format_all_color(self) -> None:
        for color, color_name in self.color_list:
            self.create_format(
                color,
                color_name,
                self.A6SLIM_PORTRAIT_WIDTH,
                self.A6SLIM_PORTRAIT_HEIGHT
            )


if __name__ == "__main__":
    format = BaseNoteFormat("A6SlimDotGrid")
    format.create_format(
        colors.HexColor("#84B5CF"),
        "_白群（びゃくぐん）",
        format.A6SLIM_PORTRAIT_WIDTH,
        format.A6SLIM_PORTRAIT_HEIGHT
