...
for color, colorName in colorsList:
    # 出力先設定
    outPath = "./pdf/A5Slim-Dot-Daily2-Color"
    ...
    for x, y in points1st:
        pdfFile.circle((x+page_width*0)*mm, (a4height-y)*mm,
                       dotsize1st*mm, stroke=0, fill=1)
        pdfFile.circle((x+page_width*1)*mm, (a4height-y)*mm,
                       dotsize1st*mm, stroke=0, fill=1)
        pdfFile.circle((x+page_width*2)*mm, (a4height-y)*mm,
                       dotsize1st*mm, stroke=0, fill=1)
    ...
    for x, y in points2nd:
        ...
        pdfFile.circle((x+page_width*2)*mm, (a4height-y)*mm,
                       dotsize2nd*mm, stroke=0, fill=1)
    # 日付領域
    header_mergin_top = dotspace2nd
    header_mergin_left = dotspace2nd
    header_mergin_bottom = dotspace2nd
    header_mark_height = dotspace2nd
    header_mark_width = dotspace2nd
    ...
    # タイムライン
    interval = dotspace2nd
    timeline_mergin_left = dotspace2nd*2
