...
class BaseNoteFormat:
    ...
    def set_property(self):
        ...
        self.pdf_file.setPageSize(
            (self.PAGE_WIDTH*mm, self.PAGE_HEIGHT*mm))
    ...
    def draw_first_dotgrid(self, color):
        ...
        points1st = [((x)*self.FIRST_DOT_INTERVAL, (y)*self.FIRST_DOT_INTERVAL)
                     for x in range(math.ceil(self.PAGE_WIDTH
                                              / self.FIRST_DOT_INTERVAL))
                     for y in range(math.ceil(self.PAGE_HEIGHT
                                              / self.FIRST_DOT_INTERVAL) + 1)]
        for x, y in points1st:
            self.pdf_file.circle(x * mm,
                                 (self.PAGE_HEIGHT - y) * mm,
...
if __name__ == "__main__":
    format = BaseNoteFormat("Single-DotGrid")
    format.create_format(colors.slateblue, "slateblue")
